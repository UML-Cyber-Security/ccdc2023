        ...
        for usr in usr_accts:
            if usr[0] == 'root':
                ...
                while (resp != "yes" and resp != "no"):
                   resp = input(f"Would you like to change the password for user (if yes, type 'yes', if no type 'no'): {usr[0]}? ")
            ...
            if usr[1] == '0':
                resp = ''
                while (resp != 'yes' and resp != 'no'):
                    resp = input(f"UID 0 found for user {usr[0]}, would you like to change: ")
                if resp == 'yes':
